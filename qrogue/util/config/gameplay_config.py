...
class OptionsType(Enum):
    ...
    def get_callbacks(self, data: Any) -> Tuple[Callable[[int], str], Callable[[str], Any]]:
        ...
        raise Exception("Invalid OptionsType!")
...
class GameplayConfig:
    ...
    def to_file_text() -> str:
        ...
        for option in GameplayConfig.__OPTIONS:
            cur_index = GameplayConfig.__OPTIONS[option]
            text += f"{option.name}{GameplayConfig.__KEY_VALUE_SEPARATOR}{option.convert(cur_index)}"
            text += "\n"
        return text
    ...
    def from_log_text(log_text: str) -> bool:
        ...
        for line in log_text.splitlines():
            if len(line.strip(" ")) == 0:
                continue
            try:
                name, value = line.split(GameplayConfig.__KEY_VALUE_SEPARATOR)
                name = normalize(name)
                for option in Options:
                    if normalize(option.name) == name:  # compare the normalized names
                        index = GameplayConfig._get_closest_index(option, value)
                        GameplayConfig.__OPTIONS[option] = index
                        break
            except IndexError:
                return False
            except KeyError:
